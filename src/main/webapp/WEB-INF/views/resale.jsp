...
<head>
    ...
    <style type="text/css">
        .panel-height {
            height: 600px; / change according to your requirement/
        }
...
<div class="page-content">
    ...
    <div class="panel panel-default">
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                <c:forEach items="${resales}" var="item">
                    <tr>
                        <td>${item.id}</td>
                        <td>${item.name}</td>
                        <td>${item.phone}</td>
                        <td>${item.originLocation}</td>
                        <td>${item.resaleLocation}</td>
    ...
    <!-- Default panel -->
    <div class="panel panel-default">
        <div class="panel-heading"><h6 class="panel-title">数据可视化</h6>
        </div>
        <div class="alert alert-warning fade in ">
            <button type="button" class="close" data-dismiss="alert">×</button>
            <i class="icon-info"></i> <span id="mapdata">北京：25辆车</span>
        </div>
...
<script type="text/javascript">
    ...
    var svg = d3.select("#xunjiaLink").insert("svg:svg", "h2")
            .attr("width", w)
            .attr("height", h);
    ...
    d3.csv("/js/china.csv", function(airports) {
        ...
        g.append("svg:path")
                ...
                .on("mouseover", function(d, i) { d3.select("#mapdata").text(d.Airport+": 发布25辆车，均价12W；转入28辆，均价13W"); });
        ...
        var randR = d3.random.normal(15,25);
    ...
    var chart = c3.generate({
        ...
        data: {
            ...
            columns: [
                ...
                ['转发次数'<c:forEach var="item" items="${chartData}">,${item.value}</c:forEach>],
                ['转发人数'<c:forEach var="item" items="${chartUserData}">,${item.value}</c:forEach>]
            ]
        },
        axis: {
            x: {
